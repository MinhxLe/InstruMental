...
class VectorMachine:
    def __init__(self, basewidth=100):
    ...
    def readImages(self, endInd, emoDict, path="", imgType=".jpg", startInd=0):
        '''
        ...
        '''
        images = np.zeros((1,self.basewidth**2))
        y = []
        for i in range(startInd, endInd):
            for key, val in emoDict.items():
                try:
                    pic = self.getCompressedRaveledPic(path, key, imgType, str(i), "compressed")
                    images = np.append(images, [pic], axis=0)
                    y.append(val)
                except OSError:
    ...
    def getCompressedRaveledPic(self, path, key, imgType, i, postfix):
        img = Image.open(path+key+i+imgType.lower())
        img = img.resize((self.basewidth, self.basewidth))
        img.save(path+key+i+postfix+imgType.lower())
        pic = cv2.imread(path+key+i+postfix+imgType.lower(), 0)
        return pic.ravel()
